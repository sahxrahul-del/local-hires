...
import { 
  ...
  ShieldCheck, GraduationCap, Settings, Hash, Globe, Lock
} from 'lucide-react';
...
export default function Profile() {
  ...
  const [isGoogleLogin, setIsGoogleLogin] = useState(false); // 🟢 State for Google Check
  
  const [formData, setFormData] = useState({
    id: '', email: '', full_name: '', phone: '', role: 'seeker', avatar_url: '',
    province: '', district: '', city_zone: '', address: '', ward: '', tole: '', 
    business_name: '', business_type: '', business_address: '', business_email: '', 
    pan_number: '', website: '' 
  });
  ...
  useEffect(() => {
    const fetchProfile = async () => {
        try {
          ...
          // Supabase stores the provider in app_metadata.providers array
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updates = {
          ...
          city_zone: (isSeeker) ? formData.city_zone : null, 
          address: (isSeeker) ? formData.address : null,
          ward: (isSeeker || isBusiness) ? formData.ward : null, 
          tole: (isSeeker || isBusiness) ? formData.tole : null, 
          ...
          website: isBusiness ? formData.website : null, 
      };
  ...
  const getTheme = (role) => {
    switch(role) {
      case 'admin': return { banner: 'bg-red-900', badge: 'bg-red-50 text-red-700 border-red-100', icon: ShieldCheck, label: 'Administrator', alert: 'bg-red-50 text-red-900 border-red-200' };
      case 'business': return { banner: 'bg-blue-900', badge: 'bg-blue-50 text-blue-700 border-blue-100', icon: Building2, label: 'Business Owner', alert: 'bg-blue-50 text-blue-900 border-blue-200' };
      case 'tuition_manager': return { banner: 'bg-purple-900', badge: 'bg-purple-50 text-purple-700 border-purple-100', icon: GraduationCap, label: 'Tuition Manager', alert: 'bg-purple-50 text-purple-900 border-purple-200' };
      case 'business_manager': return { banner: 'bg-indigo-900', badge: 'bg-indigo-50 text-indigo-700 border-indigo-100', icon: Briefcase, label: 'Business Manager', alert: 'bg-indigo-50 text-indigo-900 border-indigo-200' };
      case 'business_tuition_manager': return { banner: 'bg-amber-800', badge: 'bg-amber-50 text-amber-800 border-amber-100', icon: Settings, label: 'Operations Manager', alert: 'bg-amber-50 text-amber-900 border-amber-200' };
      default: return { banner: 'bg-slate-700', badge: 'bg-gray-100 text-gray-700 border-gray-200', icon: User, label: 'Job Seeker', alert: 'bg-gray-50 text-gray-900 border-gray-200' };
    }
  };
      ...
      <main className="max-w-4xl mx-auto mt-8 px-4 sm:px-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <div className="px-8 pb-8">
            ...
            {message && (
              <div className={`p-4 rounded-xl mb-6 flex items-start ${message.type === 'success' ? 'bg-green-50 text-green-900 border border-green-100' : 'bg-red-50 text-red-900 border border-red-100'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl mx-auto">
                  ...
                  <section className="space-y-6 animate-in fade-in">
                      ...
                      <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <input type="text" name="address" value={formData.address} onChange={handleChange} className={inputClass} placeholder="Street Address" required />
                           </div>
                           <div className="grid grid-cols-2 gap-4 mt-4">
                                <div>
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Ward No.</label>
                                    <input type="text" name="ward" value={formData.ward} onChange={handleChange} className={inputClass} placeholder="e.g. 4" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 mb-1 block">Tole / Area</label>
                                    <input type="text" name="tole" value={formData.tole} onChange={handleChange} className={inputClass} placeholder="e.g. Bhanu Chowk" />
                                </div>
                           </div>
                      </div>
                  </section>
              ...
              {isBusiness && (
                  <section className="space-y-6 animate-in fade-in">
                      <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider border-b pb-2"><Building2 className="w-4 h-4 mr-2 inline" /> Company Info</h3>
                      <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <input type="text" name="pan_number" value={formData.pan_number} onChange={handleChange} className={inputClass} placeholder="123456789" />
                          </div>
                      </div>
                      
                      <div>
                        <label className={labelClass}>Website / Social Page <span className="text-gray-400 text-xs">(Optional)</span></label>
                        <div className="relative">
                            <Globe className="absolute left-3.5 top-3.5 w-5 h-5 text-gray-400" />
                            <input type="text" name="website" value={formData.website} onChange={handleChange} className={`${inputClass} pl-11`} placeholder="https://..." />
                        </div>
                      </div>
                      ...
                      <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                           ...
                           <input type="text" name="business_address" value={formData.business_address} onChange={handleChange} className={inputClass} placeholder="Full Address (e.g. Bhanu Chowk)" />
                           <div className="grid grid-cols-2 gap-4 mt-4">
                                <input type="text" name="ward" value={formData.ward} onChange={handleChange} className={inputClass} placeholder="Ward No." />
                                <input type="text" name="tole" value={formData.tole} onChange={handleChange} className={inputClass} placeholder="Tole / Area" />
                           </div>
                      </div>

                      <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider border-b pb-2 pt-4"><User className="w-4 h-4 mr-2 inline" /> Owner Contact</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} className={inputClass} required placeholder="Owner Name" />
                          <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className={inputClass} required placeholder="Phone" />
                      </div>
                      <input type="email" name="business_email" value={formData.business_email} onChange={handleChange} className={inputClass} placeholder="Public Email (Optional)" />
                  </section>
              )}

              {/* MANAGER & ADMIN PROFILE - REVERTED TO SIMPLER STYLE */}
              {isManagerOrAdmin && (
                  <section className="space-y-6 animate-in fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div>
                            ...
                            <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} className={inputClass} required placeholder="Your Name"/>
                          </div>
                          <div>
                            <label className={labelClass}>Contact Mobile <span className="text-red-500">*</span></label>
                            <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className={inputClass} required placeholder="9800000000" />
                          </div>
                      </div>
                  </section>
              )}
              ...
              {/* 🟢 ACTION BUTTONS - NOW WITH CHANGE PASSWORD CONDITION */}
              <div className="pt-2 flex flex-col md:flex-row gap-4 items-center">
                <button type="submit" disabled={saving} className="w-full md:w-auto flex-1 bg-blue-900 text-white font-bold py-4 rounded-xl hover:bg-blue-800 transition shadow-lg flex justify-center items-center text-lg">
                  {saving ? "Saving..." : "Save Profile"}
                </button>

                {/* 🔒 Show Only if NOT Google Login */}
                {!isGoogleLogin && (
